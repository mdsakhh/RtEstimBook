...
<p><span class="bold-text" style="font-size: 1.5em;">This document helps visualize the various steps occurring in an infectious disease outbreak.</span></p>

We take several steps to simulate how cases spread in a population: 

1. We simulate individual-level infectious disease dynamics
4. We then simulate the population-level infectious disease dynamics, via $R(t)$.

<div style="border: 2px dotted black; border-radius: 15px;padding: 5px; background-color: lightyellow;">
<h3 style="margin-top: 5px; font-size: 1.25em;">Goal of this simulation:</h3>
With these factors together, we can see how a disease outbreak gets reported to administrative databases, and see how $R(t)$ can be used to inform public health decision making.
</div>

### Overview of individual-level disease dynamics

This figure describes individual-level infectious disease dynamics:

```{r schema1, echo=FALSE, out.width = '100%'}
knitr::include_graphics("img/Schema1.png")
```

* The **incubation time** distribution, the time between between infection and sympton onset (/when one person can infect another)

* The **transmission time** distribution, defined as the time between symptom onset in one person and infection in another.

* The **generation interval** , the time between infections from one person to the next. 

* The **serial interval**, the time between symptom onsets from one person to the next. Note that when transmission time and incubation time are assumed to be independent, the generation interval and the serial interval are equivalent.

* The **time-to-test delay**, the time between symptom onset for an individual and taking a diagnostic test 

* The **administrative delay**, the time it takes a positive test to appear in surveillance data
...
In each of the distribution graphs below, the y-axis says Probability. This is the probability of an individual having a incubation time (or other distribution) on that specific day. So the most number of people will have a incubation time where Probability is the highest
</div>

### Incubation time distribution
Incubation time is defined as the time between **infection** and **symptom onset** for an individual. 

```{r incb, echo=FALSE, out.width = '40%'}
knitr::include_graphics("img/Incubation.png")
```

The input panel below defines the parameters for the incubation time distribution. You can vary the average incubation time, the maximum incubation time, and the "spread" of the distribution, which controls the shape of the distribution around the mean. 
...
inputPanel(
  # row 1
  sliderInput("inc_i_mean", label = "Average (days)", value = 2, min = 1, max = 15),
    sliderInput("maxinc", label = "Maximum (days)", value = 11,
              min = 5, max = 15, step = 1),
  sliderTextInput("inc_i_size", label = "Distribution spread", selected = "More", choices = names(SIZE_LOOKUP), grid = T)

)

inc_data <- reactive({
  
  INC_I_SIZE_num = SIZE_LOOKUP[input$inc_i_size]
  
  data.frame(x = sample(0:input$maxinc, 
                          size = 5000, 
                          prob = dnbinom(0:input$maxinc, 
                                         size = 1*10^INC_I_SIZE_num, 
                                         mu = input$inc_i_mean), 
...
Transmission time is defined as the time beteween between **symptom onset** in **infector** and **infection** in **infectee** for an individual. 

```{r trans, echo=FALSE, out.width = '40%'}
knitr::include_graphics("img/Transmission.png")
```

The input panel below defines the parameters for the transmission time distribution. You can vary the average transmission time, the maximum transmission time, and the "spread" of the distribution, which controls the shape of the distribution around the mean. In this simulation we assume that the transmission time distribution is **independent** from the incubation time distribution. 
...
inputPanel(
  ...
  sliderInput("trans_i_mean", label = "Mean (days)", value = 1, min = 0, max = 15),
    sliderInput("maxtrans", label = "Maximum (days)", value = 10,
              min = 5, max = 15, step = 1),
  sliderTextInput("trans_i_size", label = "Distribution spread", 
                  choices = names(SIZE_LOOKUP), grid = T,
                  selected = "More", width = "90%")

)

trans_data <- reactive({
    TRANS_I_SIZE_num = SIZE_LOOKUP[input$trans_i_size]
  
      data.frame(x = sample(0:input$maxtrans, 
                          size = 5000, 
                          prob = dnbinom(0:input$maxtrans, 
                                         size = 1*10^TRANS_I_SIZE_num, 
...
Generation time is the time between **infection** in the **infector** and **infection** in the **infectee**, 

```{r gen, echo=FALSE, out.width = '60%'}
knitr::include_graphics("img/Generation.png")
```

Changes above to the incubation time or transmission time will affect this distribution. In this simulation we assume that the transmission time distribution is **independent** from the incubation time distribution, so the generation time distribution and serial interval distribution (below) are the same.
...
Serial interval is the time between **symptom onset** in the **infector** and **symptom onset** in the **infectee**

```{r serial, echo=FALSE, out.width = '60%'}
knitr::include_graphics("img/Serial.png")
```

Changes above to the incubation time or transmission time will affect this distribution. In this simulation we assume that the transmission time distribution is **independent** from the incubation time distribution, so the generation time distribution (above) and serial interval distribution are the same.
...
Finally, we model a reporting delay distribution, the time between **symptom onset** and **case reporting** to a public health agency. **In real-world settings this is likely two separate distributions**: one for how long it takes someone to get a positive test once their symptoms show, and the other for how long that positive test takes to be recorded in an administrative dataset.

```{r admin, echo=FALSE, out.width = '60%'}
knitr::include_graphics("img/Admin.png")
```

For the purpose of demonstration, we lump these together. You can vary the average reporting delay time, the maximum reporting delay time, and the "spread" of the distribution, which controls the shape of the distribution around the mean.
...
inputPanel(
  ...
  sliderInput("maxdelay", label = "Maximum (days)", value = 11,
              min = 10, max = 15, step = 1),
    sliderTextInput("NB_r", label = "Distribution spread", 
                  choices = names(SIZE_LOOKUP), grid = T,
                  selected = "More", width = "90%")
)
...
reporting_dist <- reactive({
      ...
      REPORT_I_SIZE_num = SIZE_LOOKUP[input$NB_r]
  
      data.frame(x = sample(0:input$maxdelay, 
                          size = 5000, 
                          prob = dnbinom(0:input$maxdelay, 
                                         size = 1*10^REPORT_I_SIZE_num, 
                                         mu = input$NB_m), 
                          replace = TRUE) ) %>%
      group_by(x) %>% tally() %>%
    mutate(n = n / sum(n))
...
We first describe the daily infections, onsets, and reporting days of the first generation, i.e. the first round of infectees in the data. For example, this could be people that were exposed at a conference out of state that then traveled home and are now reporting in their home state. 

```{r init1, echo=FALSE}
inputPanel(
    sliderInput("n0", label = "Initial cases:", width = '90%',min = 100, max = 1000, value = 500, step = 100)
)
```

With these number of initial cases, and the distributions defined above, we can observe the timing of the first generation of daily infections, disease onsets, and administrative reports. Notice the differences in the distributions: the daily infections reflects the transmission time, the daily onsets reflects the incubation time, and the daily reports reflects the administrative delay.
...
inputPanel(
      ...
      radioButtons('CurveType', 'Curve type', choices = c('Linear', 'Smooth'))
)
...
rt_grid <- reactive({
  ...
  X_mat <- bs(x = this.x, knots = rv$x, degree = BSDEG)
  m2 <- lm(this.y ~ X_mat)
  yspline <- predict(m2)
...
Now, create the all subsequent generations within the modeling window (generation # is indicated by fill color in the graph below, with lighter being later generations. The first generation is outlined in <span style="color: red;"><strong>red</strong></span>). 
...
<div style="border: 2px dotted black; border-radius: 15px;padding: 5px; background-color: lightyellow;">
<h3 style="margin-top: 5px; font-size: 1.25em;">Right truncation:</h3>
Importantly, you may look at the daily reports and notice that the end of the graph and notice that the daily reports trail off as you approach “present day”. This is a phenomena known as **right-truncation**, and is due to the reporting delay distribution. At any point in time, due to the administrative delay in reporting, not all recent reports have been entered into the database. After sufficient time has passed these reports will come in but at the present time there is always a lag. The <span style="color: purple;"><strong>purple line</strong></span> represents reports that will eventually be captured.  
</div>
<br>
Here is another way to visualize right truncation: you can change the present day and view the reports that will have come in on that day as a function of the reporting delay. The <span style="color: red;"><strong>red line</strong></span> represents the reports that are actually captured as a function of time, and the <span style="color: grey;"><strong>grey line</strong></span> represents reports that will eventually be captured. 
